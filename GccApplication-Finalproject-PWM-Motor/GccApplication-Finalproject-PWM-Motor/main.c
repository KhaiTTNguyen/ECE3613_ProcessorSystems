...
//Example 2. DC motor PWM - Duty cycle control using Timer1 compare match mode interrupt
#define F_CPU 16000000
#include <avr/io.h>
#include <avr/interrupt.h>
...
void OLED_display(){	//this will update the OLED display with the most current info
	OLED_SetCursor(0, 0);
	OLED_Printf("Sensor Reading: ");
	OLED_DisplayNumber(C_DECIMAL_U8,Sensor_counter,5);
	OLED_SetCursor(2, 0);
	OLED_Printf("Timer: ");
	OLED_DisplayNumber(C_DECIMAL_U8,counter,6);
	OLED_SetCursor(4, 0);
	OLED_Printf("RPM: ");
...
ISR(TIMER0_COMPA_vect){	//ISR for when timer0 overflows
	...
	int k=1; //number of sensor reading per 1 revolution, if you have 2 wheel opening, change to 2
